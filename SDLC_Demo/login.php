...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    /*// Tài khoản admin khởi tạo tự động nếu chưa có
    if ($user === 'Admin' && $pass === 'admin123') {
        $check = mysqli_query($connect, "SELECT * FROM users WHERE Username='Admin'");
        if (mysqli_num_rows($check) == 0) {
            $hash = password_hash('admin123', PASSWORD_DEFAULT);
            mysqli_query($connect, "INSERT INTO users (RoleID, Name, Email, Phone, Address, Username, Password)
                VALUES (1, 'Admin', 'superadmin@example.com', '0123456789', 'Admin Address', 'Admin', '$hash')");
        }
        $_SESSION['user'] = 'Admin';
        $_SESSION['role'] = 1;
        echo "<script>alert('Đăng nhập admin khởi tạo!'); window.location.href = 'admin/index.php';</script>";
        exit();
    }*/
